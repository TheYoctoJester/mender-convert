...
# Compatible with Python 3.7+ - removed encoding parameter for broader compatibility
logging.basicConfig(filename=os.path.join(root_path, persistent_directory, "validation.log"), level=logging.DEBUG)
logger.info("Mender validation script started {}".format(datetime.datetime.now()))
###############################################################################
...
class PersistentState:
    ...
    def __init__(self, logger: logging.Logger, root_directory="/", persistent_directory="data"):
        ...
        try:
            if config_a is None:
                self.logger.info("root filesystem A not found in  config, aborting")
                raise RuntimeError()
            if config_b is None:
                self.logger.info("root filesystem B not found in  config, aborting")
                raise RuntimeError()
            if last_a is not None and last_a != config_a:
                self.logger.info("root filesystem A in config {} does not match last used one {}, aborting".format(config_a, last_a))
                raise RuntimeError()
            if last_b is not None and last_b != config_b:
                self.logger.info("root filesystem B in config {} does not match last used one {}, aborting".format(config_b, last_b))
                raise RuntimeError()
        except RuntimeError:
            print("triggering config:\n{}, triggering state:\n{}".format(json.dumps(self.config, indent=2), json.dumps(self.state, indent=2)))
            raise RuntimeError()
        self.logger.info("...state and config ready.")
    ...
    def create_initial_state(self):
        ...
        self.logger.info("resulting in {}".format(json.dumps(self.config, indent=2)))
        ...
        self.logger.info("resulting in {}".format(json.dumps(self.state, indent=2)))
    ...
    def next_step(self):
        s = self.get_step()
        self.logger.info("proceeding to next step from {}".format(s))
        if s == self.STEP_NONE:
            ...
            self.logger.error("could not get next step after {}, aborting".format(s))
            raise Exception
        s = self.get_step()
        self.logger.info("new step is {}".format(s))
        return s
...
def identify_mounted_root(state: PersistentState):
    ...
    logger.info("mount identification -  '/': {}, '{}': {}, '{}': {}, result is {}".format(root, state.get_root_part_device_a(), a, state.get_root_part_device_b(), b, result))
    return result
...
def assert_env_variable(state: PersistentState, variable_name: str, expect: str):
    ...
    logger.info("checking env {} for {}, cmd result {}".format(variable_name, expect, value))
    if value is None:
        return False

    vdict = dict(re.findall(r"^\s*(.*?)\s*=\s*(.*?)\s*$", value))
    logger.info("evaluation output to {}".format(vdict))
...
s = state.get_step()
logger.info("starting evaluation of step {}".format(s))
if s == None:
    ...
    expected = state.get_expected_root()
    if expected != current_root:
        fail_reason = "rollback test did not match expected root: {}".format(expected)
        logger.info(fail_reason)
        keep_going = False
...
logger.info("ending evaluation of step {}".format(s))
###############################################################################
...
logger.info("starting prepartion of step {}".format(s))
if s == state.STEP_INIT:
    logger.info("gathering some system information")
    logger.info("uname -a: {}".format(run_command_get_output(["uname", "-a"])))
    with open('/etc/os-release', 'r') as file:
        logger.info("/etc/os-release:\n{}".format(file.read()))
elif s == state.STEP_TEST_SWITCH:
    ...
    if inactive is not None:
        if set_mender_bootpart(state, inactive[INACTIVE_PART_NUMBER]):
            ...
            fail_reason = "failed to set boot partition {}".format(inactive[INACTIVE_PART_IDENT])
            logger.info(fail_reason)
            keep_going = False
    else:
    ...
    if keep_going and inactive is not None:
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            if not run_command(["mount", inactive[INACTIVE_PART_DEVICE], tmpdirname]):
                fail_reason = "failed to mount {} to {}, aborting".format(inactive[INACTIVE_PART_DEVICE], tmpdirname)
                logger.info(fail_reason)
                keep_going = False
            ...
            if keep_going:
                try:
                    ...
                    fail_reason = "failed to rename {} in {}, aborting".format(BOOT_DIRECTORY, tmpdirname)
                    logger.info(fail_reason)
                    keep_going = False
            # 4: unmount inactive partition
            if keep_going and not run_command(["umount", tmpdirname]):
                fail_reason = "failed to unmount {}".format(tmpdirname)
                logger.info(fail_reason)
                keep_going = False
    ...
    logger.info("ending")
    keep_going = False
logger.info("ending prepartion of step {}".format(s))
###############################################################################
...
if keep_going:
    ...
    run_command(["systemctl", "disable", "bootloader-validation.service"])
    state.clean()
    if fail_reason == None:
        logger.info("BOOTLOADER VALIDATION: SUCCESS")
    else:
        logger.info("BOOTLOADER VALIDATION: FAILURE - {}".format(fail_reason))